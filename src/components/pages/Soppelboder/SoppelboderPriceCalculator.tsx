...
const BIN_PRESETS: Record<number, { width: number; depth: number; height: number }> = {
  1: { width: 70, depth: 60, height: 160 },
  2: { width: 140, depth: 60, height: 160 },
  3: { width: 210, depth: 60, height: 160 },
  4: { width: 280, depth: 60, height: 160 },
  5: { width: 350, depth: 60, height: 160 },
}
...
export default function SoppelboderPriceCalculator({ basePrice, onConfigChange }: SoppelboderPriceCalculatorProps) {
  ...
  const handleBinCountChange = useCallback((count: number) => {
    setBinCount(count)
    const preset = BIN_PRESETS[count]
    setWidth(preset.width)
    setHeight(preset.height)
    setDepth(preset.depth)
  }, [])

  // When width changes manually, sync bin count to match
  const handleWidthChange = useCallback((w: number) => {
    setWidth(w)
    const binsFromWidth = Math.min(5, Math.max(1, Math.floor(w / 70)))
  ...
  const WIDTH_PER_CM = 3000 / BASE_WIDTH   // ~43 kr/cm
  const DEPTH_PER_CM = 3000 / BASE_DEPTH   // ~50 kr/cm per 10cm ≈ 500 kr
  const HEIGHT_PER_CM = 3000 / BASE_HEIGHT // ~19 kr/cm per 10cm ≈ 188 kr
  ...
  const installationCost = installationChecked ? 3500 : 0
  const lightingCost = lightingChecked ? 1700 * binCount : 0

  const subtotal = volumeCost + finishCost + roofCost + qualityCost + deliveryCost + installationCost + lightingCost
  ...
  const handleAddToBasket = () => {
    addItem({
      ...
      lighting: lightingChecked ? `Ja (${binCount} downlights)` : 'Nei',
      size: `${binCount} dunker`,
      complexity: doorLabel(doorType),
    ...
    <CalculatorContainer>
      ...
      <SectionTitle>
        ...
        <TooltipWrapper>
          ...
          <TooltipContent>
            Velg hvor mange søppeldunker boden skal romme. Målene justeres automatisk.
          </TooltipContent>
        </TooltipWrapper>
      </SectionTitle>
      <SelectWrapper>
      ...
      <SectionTitle>
        ...
        <TooltipWrapper>
          <Icon name="faInfoCircle" />
          <TooltipContent>
            Målene er forhåndsinnstilt basert på antall dunker, men kan justeres.
          </TooltipContent>
        </TooltipWrapper>
      </SectionTitle>
      <SliderGroup>
      ...
      <CheckboxBox $checked={lightingChecked}>
        ...
        Lysinstallasjon (+{(1700 * binCount).toLocaleString('nb-NO')},- per dunk)
      </CheckboxBox>
...
function doorLabel(value: string): string {
  switch (value) {
    case 'top':
      return 'Topplokk'
    case 'both':
      return 'Frontdør + topplokk'
    default:
      return 'Frontdør'
  }
}
