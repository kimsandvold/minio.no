...
export interface PostkassestativConfig {
  bredde: number // cm (x)
  dybde: number // cm (z)
  ...
  bunnskap: boolean // rom/skap i bunnen (60 cm høye) med terrassebord-hylle + skillevegger
  postkasser: boolean // vis postkasser (én over hvert rom) på innervegg
  treslag: string
  farge: string
...
// Bunnskap: 70 cm høye rom, tynn ramme/skillevegg (21 mm bord).
const SKAP_H = 0.7
const SKAP_T = 0.021
// Antall rom: hvert rom ≥ ~50 cm; antallet øker med innvendig bredde.
function antallSkap(innerW: number): number {
  return Math.max(1, Math.floor(innerW / 0.5))
}
...
function takUnderkant(c: PostkassestativConfig, x: number, z: number, w: number, d: number, topY: number): number {
...
// Kledningsbord på bakvegg + sider (fronten står åpen for postkassene). Hvert
// bord kappes til takfallet (topp = takUnderkant på bordets senter).
function kledningBoards(c: PostkassestativConfig, w: number, d: number, topY: number, botWall: number) {
  const ks = kledningSpec(c.kledningsbord)
  const kt = ks.t
  const topGap = 0.03 // klaring så bordtoppen ligger under sperra (ikke stikker opp)
  const SPRKK = 0.003
  ...
  // Front: hjørnene kles i FULL høyde (dekker hjørnestolpene sømløst, bordene
  // går forbi sidekledningen ±kt). Fronttoppen over åpningen dekkes helt til
  // taket. Selve postkasse-/rom-åpningen i midten står åpen for tilgang.
  const tbW = c.bredde
  const topBeamH = tbW <= 240 ? 0.148 : tbW <= 360 ? 0.198 : 0.223
  const frontWallTop = topY - topBeamH
  const jamb = w / 2 - ST // innerkant hjørnestolpe
  const takFront = (x: number) => takUnderkant(c, clampX(x), d / 2, w, d, topY)
  run('x', d / 2 + kt / 2, -(w / 2 + kt), -jamb, botWall, takFront) // venstre hjørne
  run('x', d / 2 + kt / 2, jamb, w / 2 + kt, botWall, takFront) // høyre hjørne
  run('x', d / 2 + kt / 2, -jamb, jamb, frontWallTop, takFront) // fronttopp (midt)
  return { boards, ks }
}
...
function geo(c: PostkassestativConfig) {
...
function deler(c: PostkassestativConfig): Del[] {
  ...
  // Utvendig kledning (bak + sider – fronten står åpen for postkassene).
  ...
  // Innerkledning bak (postkasse-vegg) – fra hylle/bunn opp til taket.
  const innerW2 = w - 2 * ST
  const ksb = kledningSpec(c.kledningsbord)
  const nbInner = Math.max(1, Math.round(innerW2 / (ksb.w + 0.003)))
  const innerH = Math.max(0.1, c.hoyde * scale - (c.bunnskap ? SKAP_H : 0))
  push('Kledning (innervegg)', ksb.profil, innerH, nbInner, ksb.pris)

  // Bunnskap: terrassebord-hylle + tynne skillevegger + front-ramme.
  if (c.bunnskap) {
    const innerW = w - 2 * ST
    const innerD = d - ST - 0.01
    const nSkap = antallSkap(innerW)
    push('Terrassebord (hylle, skap)', b.profil, innerW, dekkePosisjoner(innerD, b.w).length, b.pris)
    if (nSkap > 1) push('Skillevegg (skap)', b.profil, innerD, (nSkap - 1) * Math.max(1, Math.ceil(SKAP_H / b.w)), b.pris)
    push('Ramme (skap)', b.profil, innerW, 2, b.pris)
  }
  return out
}

function beregn(c: PostkassestativConfig): Bom {
  ...
  const nPostkasser = c.postkasser ? antallSkap(c.bredde * scale - 2 * ST) : 0
  const postkasseKr = nPostkasser * prisFor('postkasse')
  const estimatKr =
    Math.round(((woodKr) * SVINN + skruer * prisFor('skrue') + nPad * prisFor('stolpesko') + takKr + postkasseKr) / 10) * 10
  ...
  if (nPostkasser > 0) linjer.push({ navn: 'Postkasse', antall: nPostkasser, enhet: 'stk', kommentar: 'én pr. rom' })
...
function kappliste(c: PostkassestativConfig): KapplisteDel[] {
...
function tegning2D(c: PostkassestativConfig): Tegning2D {
  const B = c.bredde
  const D = c.dybde
  const { nBjelke } = geo(c)
...
function buildMesh(c: PostkassestativConfig, opts?: BuildOptions): THREE.Group {
  ...
  // ── Bunnskap: 60 cm høye rom med terrassebord-hylle + tynne skillevegger ──
  if (c.bunnskap) {
    const shelfTop = deckTop + SKAP_H
    const innerW = w - 2 * ST
    const zBack = -d / 2 + ST
    const zFront = d / 2 - 0.01
    const innerD = zFront - zBack
    const zC = (zBack + zFront) / 2
    const nSkap = antallSkap(innerW)
    const skapBox = (dx: number, dy: number, dz: number, x: number, y: number, z: number, pid: string, navn: string, part: string, dark = 0.9) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(dx, dy, dz), meshMat(pid, dark))
      m.position.set(x, y, z)
      m.castShadow = true
      m.receiveShadow = true
      m.userData.part = part
      m.userData.pid = pid
      m.userData.info = { navn, profil: '21 mm bord', lengdeCm: Math.round(Math.max(dx, dy, dz) * 100) }
      group.add(m)
    }
    // Tynne skillevegger mellom rommene (kledning-lag) – fra dekket opp til hylla.
    for (let i = 1; i < nSkap; i++) {
      const x = -innerW / 2 + (i * innerW) / nSkap
      skapBox(SKAP_T, SKAP_H, innerD, x, deckTop + SKAP_H / 2, zC, `skap-vegg-${i}`, 'Skillevegg (skap)', 'kledning', 0.86)
    }
    // Tynn ramme foran: topplist (under hylla) + bunnlist (kickboard) – ramme/konstruksjon.
    skapBox(innerW, 0.07, SKAP_T, 0, shelfTop - 0.035, zFront, 'skap-rail-topp', 'Ramme (skap)', 'konstruksjon')
    skapBox(innerW, 0.07, SKAP_T, 0, deckTop + 0.035, zFront, 'skap-rail-bunn', 'Ramme (skap)', 'konstruksjon')
    // Terrassebord-hylle oppå rommene (gulv-lag – samme bord som gulvet).
    dekkePosisjoner(innerD, b.w).forEach((bp, i) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(innerW, b.t, bp.w), meshMat(`skap-hylle-${i}`, i % 2 ? 0.97 : 1))
      m.position.set(0, shelfTop + b.t / 2, zC + bp.z)
      m.castShadow = true
      m.receiveShadow = true
      m.userData.part = 'gulv'
      m.userData.pid = `skap-hylle-${i}`
      m.userData.info = { navn: 'Terrassebord (hylle)', profil: b.profil, lengdeCm: Math.round(innerW * 100) }
      group.add(m)
    })
  }

  // ── Innervegg (bak) + postkasser ──
  // Bakveggen kles innvendig der postkassene monteres. Med bunnskap starter
  // innerkledningen på hylla (60 cm); uten bunnskap går den helt til bunnen.
  {
    const innerW = w - 2 * ST
    const nSkap = antallSkap(innerW)
    const ks = kledningSpec(c.kledningsbord)
    const kt = ks.t
    const backInnerZ = -d / 2 + ST // innerside bak-stenderne
    const sidingZ = backInnerZ + kt / 2
    const botY = c.bunnskap ? deckTop + SKAP_H : deckTop
    const SPRKK = 0.003
    const n = Math.max(1, Math.round(innerW / (ks.w + SPRKK)))
    const step = innerW / n
    const bredde = step - SPRKK
    for (let i = 0; i < n; i++) {
      const x = -innerW / 2 + step * (i + 0.5)
      // Innerveggen (montasjeflate for postkasser) stopper ved veggtoppen –
      // aldri over. Taket ligger alltid ≥ veggtoppen, så den kan ikke stikke opp.
      const top = Math.min(takUnderkant(c, x, backInnerZ, w, d, yTopPlate), yTopPlate) - 0.03
      const h = top - botY
      if (h < 0.03) continue
      const m = new THREE.Mesh(new THREE.BoxGeometry(bredde, h, kt), meshMat(`kled-inner-${i}`, 0.9))
      m.position.set(x, botY + h / 2, sidingZ)
      m.castShadow = true
      m.receiveShadow = true
      m.userData.part = 'kledning'
      m.userData.pid = `kled-inner-${i}`
      m.userData.info = { navn: 'Kledning (innervegg)', profil: ks.profil, lengdeCm: Math.round(h * 100) }
      group.add(m)
    }
    // Postkasser – klassisk norsk modell (grønn, buet topp, luke m/vrider,
    // brevsprekk + avisrør). Én over hvert rom, montert 130 cm over gulvet.
    if (c.postkasser) {
      const compW = innerW / nSkap
      const bw = Math.min(0.30, compW * 0.8) // stående (portrett) korpus
      const bh = 0.42
      const bd = Math.min(0.34, d - ST - kt - 0.06)
      // Ideell montering 130 cm over gulv. Er veggen for lav (eller hylla i
      // veien), senkes kassa så toppen holder seg under veggtoppen og over
      // hylla/gulvet.
      const boxTotalH = bh + 0.09 // korpus + buet topp
      const maxBot = yTopPlate - boxTotalH - 0.05 // holdes under veggtoppen
      const minBot = (c.bunnskap ? deckTop + SKAP_H : deckTop) + 0.05 // over hylle/gulv
      const boxBot = Math.max(minBot, Math.min(deckTop + 1.3, maxBot))
      const frontZ = backInnerZ + kt // innervegg-flate
      const cz = frontZ + bd / 2
      const front = frontZ + bd // frontflate
      const green = new THREE.MeshStandardMaterial({ color: 0x3a7043, roughness: 0.45, metalness: 0.2 })
      const greenDark = new THREE.MeshStandardMaterial({ color: 0x2c5834, roughness: 0.45, metalness: 0.2 })
      const dark = new THREE.MeshStandardMaterial({ color: 0x121212, roughness: 0.5 })
      const addPart = (geo: THREE.BufferGeometry, matl: THREE.Material, x: number, y: number, z: number, pid: string, navn: string, rot?: [number, number, number], sc?: [number, number, number]) => {
        ...
        m.castShadow = true
        m.receiveShadow = true
        m.userData.part = 'postkasse'
        m.userData.pid = pid
        m.userData.info = { navn, profil: '', lengdeCm: 0 }
        group.add(m)
      }
      for (let j = 0; j < nSkap; j++) {
        const cx = -innerW / 2 + compW * (j + 0.5)
        // Korpus (stående)
        addPart(new THREE.BoxGeometry(bw, bh, bd), green, cx, boxBot + bh / 2, cz, `postkasse-${j}`, 'Postkasse')
        // Buet topp med lite utstikk foran (halvsylinder på tvers, flatet)
        addPart(new THREE.CylinderGeometry(bd / 2, bd / 2, bw + 0.01, 20), green, cx, boxBot + bh, cz + 0.008, `postkasse-${j}-topp`, 'Postkasse (topp)', [0, 0, Math.PI / 2], [0.5, 1, 1.08])
        // Brevinnkast øverst: lite lokk + sprekk
        addPart(new THREE.BoxGeometry(bw * 0.66, bh * 0.09, 0.01), greenDark, cx, boxBot + bh * 0.85, front + 0.005, `postkasse-${j}-innkast`, 'Postkasse (innkast)')
        addPart(new THREE.BoxGeometry(bw * 0.52, 0.016, 0.012), dark, cx, boxBot + bh * 0.85, front + 0.011, `postkasse-${j}-sprekk`, 'Postkasse (brevsprekk)')
        // Stor luke (dør) med opphøyd rammepanel
        addPart(new THREE.BoxGeometry(bw * 0.86, bh * 0.58, 0.008), green, cx, boxBot + bh * 0.33, front + 0.004, `postkasse-${j}-luke`, 'Postkasse (luke)')
        addPart(new THREE.BoxGeometry(bw * 0.66, bh * 0.44, 0.006), greenDark, cx, boxBot + bh * 0.33, front + 0.011, `postkasse-${j}-lukepanel`, 'Postkasse (lukepanel)')
        // Kryss-vrider (butterfly) i svart – to kryssede staver
        ;[Math.PI / 4, -Math.PI / 4].forEach((a, k) =>
          addPart(new THREE.BoxGeometry(0.075, 0.016, 0.016), dark, cx, boxBot + bh * 0.66, front + 0.018, `postkasse-${j}-vrider-${k}`, 'Postkasse (vrider)', [0, 0, a]))
      }
    }
  }
  ...
  // ── Utvendig kledning (bak + sider – fronten står åpen) ──
...
export const postkassestativ: ProductTemplate<PostkassestativConfig> = {
  id: 'postkassestativer',
  navn: 'Postkassestativ',
  ikon: 'faEnvelope',
  beskrivelse: 'Tegn postkassestativet i 3D. (Under ombygging – steg 1: gulv/bjelkelag.)',
  bilde: '/images/products/postkassestativ-3d.webp',
  tilgjengelig: true,
  fraPris: 249,
  ...
  presets: [
    {
      id: 'pulttak-130',
      navn: 'Pulttak 130 cm',
      beskrivelse: 'Moderne, mørk grå – 2 rom',
      config: { bredde: 130, dybde: 60, hoyde: 200, taktype: 'pulttak', takretning: 'bak', takvinkel: 15, takpapp: true, bunnskap: true, kledningsbord: 'lekt', treslag: 'impregnert', farge: 'morkegra' },
    },
    {
      id: 'saltak-180',
      navn: 'Saltak 180 cm',
      beskrivelse: 'Klassisk, sortmalt – 3 rom',
      config: { bredde: 180, dybde: 70, hoyde: 200, taktype: 'saltak', moneretning: 'dybde', takvinkel: 22.5, takpapp: true, bunnskap: true, kledningsbord: '19x98', treslag: 'gran', farge: 'sort' },
    },
    {
      id: 'valmtak-220',
      navn: 'Valmtak 220 cm',
      beskrivelse: 'Eksklusiv lerk – 4 rom',
      config: { bredde: 220, dybde: 80, hoyde: 210, taktype: 'valmtak', takvinkel: 22.5, takpapp: true, bunnskap: true, kledningsbord: '19x148', treslag: 'lerk', farge: 'ubehandlet' },
    },
  ],
  dimensjoner: [
    { key: 'bredde', label: 'Bredde', min: 50, max: 330, step: 5, axis: 'x' },
    { key: 'dybde', label: 'Dybde', min: 35, max: 120, step: 5, axis: 'z' },
    { key: 'hoyde', label: 'Vegghøyde', min: 180, max: 250, step: 5, axis: 'y' },
  ...
  valg: [
    ...
    { key: 'bunnskap', label: 'Bunnskap (rom)', note: '70 cm høye rom med terrassebord-hylle og skillevegger (min. 50 cm brede)' },
    { key: 'postkasser', label: 'Vis postkasser', note: 'Én postkasse over hvert rom, montert på innerveggen' },
  ],
  parts: [
    { key: 'gulv', label: 'Terrassebord' },
    { key: 'konstruksjon', label: 'Ramme (bjelkelag/vegg)' },
    { key: 'kledning', label: 'Kledning' },
    { key: 'postkasse', label: 'Postkasser' },
