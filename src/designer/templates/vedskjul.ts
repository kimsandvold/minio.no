...
export interface VedskjulConfig {
  bredde: number // cm (x)
  dybde: number // cm (z)
  ...
  dorer: string // antall front-dører: '1' | '2' | '3' (skillevegg mellom hvert dørrom)
  treslag: string
  farge: string
...
// Antall front-dører (1–3). Hvert dørrom skilles med en bærende skillevegg.
function doorCount(c: VedskjulConfig): number {
  const n = parseInt(String(c.dorer ?? '2'), 10)
  return Number.isFinite(n) ? Math.min(3, Math.max(1, n)) : 2
}

// Dørramme = 48×98 (samme som stenderverket). Skråbånd holder døra i vinkel.
const DOOR_FACE = ST // rammebredde i dørplanet (98 mm)
const DOOR_T = SW // rammetykkelse (48 mm)
const DOOR_W = 1.0 // fast døråpning: 100 cm bred (karmstender i hver side)

// Døråpning + karm for ett fag: 100 cm åpning sentrert i faget (klemt så
// karmstenderne får plass), med selve dørbladet 2 cm smalere for klaring.
function doorLayout(bayW: number) {
  const openW = Math.min(DOOR_W, bayW - 2 * SW - 0.04)
  return { openW, leafW: openW - 0.02, karmDx: openW / 2 + SW / 2 }
}

// Frontveggens fulle felt VED SIDEN AV dørene (mellom dørkarm og hjørne/
// skillevegg). Her fylles veggen med stenderverk + losholt som resten av
// veggene. Returnerer senterlinjene til de omsluttende stenderne per felt.
function frontSegments(w: number, N: number): Array<{ xL: number; xR: number }> {
  const bayW = w / N
  const { karmDx } = doorLayout(bayW)
  const segs: Array<{ xL: number; xR: number }> = []
  for (let j = 0; j < N; j++) {
    const bayLeft = -w / 2 + j * bayW
    const xC = bayLeft + bayW / 2
    const leftEdge = j === 0 ? -(w / 2 - SW / 2) : bayLeft // hjørne | skillevegg
    const rightEdge = j === N - 1 ? w / 2 - SW / 2 : bayLeft + bayW
    segs.push({ xL: leftEdge, xR: xC - karmDx }) // felt venstre for døra
    segs.push({ xL: xC + karmDx, xR: rightEdge }) // felt høyre for døra
  }
  return segs
}

// ── Utvendig kledning (samme valg som søppelboden) ────────────────────────
...
// Takets underkant over et punkt (x, z) i veggflukten – kledningen kappes hit.
function takUnderkant(c: VedskjulConfig, x: number, z: number, w: number, d: number, topY: number): number {
...
// Kledningsbord på bakvegg + sider + front-feltene ved siden av dørene. Hvert
// bord kappes til takfallet (topp = takUnderkant på bordets senter). Dør-
// åpningene på fronten står åpne (klerne bare i feltene fra frontSegments).
function kledningBoards(c: VedskjulConfig, w: number, d: number, topY: number, botWall: number) {
  ...
  // Sider (venstre/høyre) – bordene går forbi kantene (±kt) for sømløse hjørner.
  run('z', -w / 2 - kt / 2, -d / 2, d / 2, botWall, (z) => takUnderkant(c, -w / 2, z, w, d, topY))
  run('z', w / 2 + kt / 2, -d / 2, d / 2, botWall, (z) => takUnderkant(c, w / 2, z, w, d, topY))
  // Bakvegg – dekker endeveden på sidekledningen (±kt).
  run('x', -d / 2 - kt / 2, -(w / 2 + kt), w / 2 + kt, botWall, (x) => takUnderkant(c, clampX(x), -d / 2, w, d, topY))
  // Front – feltene ved siden av dørene (fra bunn) …
  const N = doorCount(c)
  const bw = w / N
  frontSegments(w, N).forEach((sg) => {
    run('x', d / 2 + kt / 2, sg.xL, sg.xR, botWall, (x) => takUnderkant(c, clampX(x), d / 2, w, d, topY))
  })
  // … og feltet OVER hver dør (fra døroverkant opp til taket). Selve
  // døråpningen (under døroverkant) står åpen for døra.
  const tbW = c.bredde
  const topBeamH = tbW <= 240 ? 0.148 : tbW <= 360 ? 0.198 : 0.223
  const doorTop = topY - topBeamH // dør-overkant = underkant toppdrager
  const { karmDx } = doorLayout(bw)
  for (let j = 0; j < N; j++) {
    const xC = -w / 2 + (j + 0.5) * bw
    run('x', d / 2 + kt / 2, xC - karmDx, xC + karmDx, doorTop, (x) => takUnderkant(c, clampX(x), d / 2, w, d, topY))
  }
  return { boards, ks }
}
...
function deler(c: VedskjulConfig): Del[] {
  ...
  // ── Front: bærende skillevegger + dører (1–3) ────────────────────
  // Dørene fyller hvert fag; den doble toppdrageren i front er dragerbjelken
  // («header») over åpningene og bæres av hjørnestolpene + én bæringsstender
  // ved hver skillevegg (SINTEF Byggforsk – ingen fritt spennende drager uten
  // opplegg over hvert dørrom).
  const N = doorCount(c)
  const bayW = w / N
  const { leafW } = doorLayout(bayW)
  const dInner = d - 2 * ST
  const nDiv = N - 1
  push('Bunnsvill front (terskel)', STUD_PROFIL, w, 1, KV_BJELKE_PRIS)
  if (nDiv > 0) {
    // Bærende skillevegg mellom hvert dørrom: svill + toppsvill + stendere +
    // losholt (kryss) + én gjennomgående bæringsstender i front.
    const nz = Math.max(2, antallCC(dInner))
    const baysDiv = Math.max(1, nz - 1)
    push('Skillevegg svill/toppsvill', STUD_PROFIL, dInner, 2 * nDiv, KV_BJELKE_PRIS)
    push('Skillevegg stender', STUD_PROFIL, postH, nDiv * nz, KV_BJELKE_PRIS)
    push('Bæringsstender (front, skillevegg)', STUD_PROFIL, postH, nDiv, KV_BJELKE_PRIS)
    const losholtLenDiv = dInner / baysDiv - SW
    if (nLosholt > 0 && losholtLenDiv > 0.02) push('Losholt (kryss, skillevegg)', STUD_PROFIL, losholtLenDiv, nDiv * nLosholt * baysDiv, KV_BJELKE_PRIS)
  }
  // Front-felt ved siden av dørene: stenderverk + losholt (kryss) c/c ≤ 600.
  frontSegments(w, N).forEach((sg) => {
    const seg = sg.xR - sg.xL
    if (seg < 0.12) return
    const nb = Math.max(1, Math.ceil(seg / CC))
    if (nb - 1 > 0) push('Stender (front)', STUD_PROFIL, postH, nb - 1, KV_BJELKE_PRIS)
    const losLen = seg / nb - SW
    if (nLosholt > 0 && losLen > 0.02) push('Losholt (kryss, front)', STUD_PROFIL, losLen, nLosholt * nb, KV_BJELKE_PRIS)
  })
  // Dørkarm: bæringsstender i hver side av hver 100 cm åpning (bærer toppdrageren).
  push('Dørkarm (bæringsstender)', STUD_PROFIL, postH, 2 * N, KV_BJELKE_PRIS)
  // Dør: ramme (2 stolper + 3 tverrtre) + skråbånd, ett 100 cm blad per fag.
  const railW = Math.max(0.05, leafW - 2 * DOOR_FACE)
  push('Dørstolpe (ramme)', STUD_PROFIL, postH, 2 * N, KV_BJELKE_PRIS)
  push('Dørtverrtre (ramme)', STUD_PROFIL, railW, 3 * N, KV_BJELKE_PRIS)
  push('Skråbånd (dør)', STUD_PROFIL, Math.hypot(railW, postH), N, KV_BJELKE_PRIS)
  ...
  // Utvendig kledning (bak + sider + front-felt ved dørene).
...
function beregn(c: VedskjulConfig): Bom {
  ...
  // Montering/arbeidstid: 16 t (liten, enkel) → 80 t (stor og kompleks).
  // Basert på faktisk byggetid; skaleres av grunnflate + taktype + antall dører
  // (hver ekstra dør = ekstra dørrom + bærende skillevegg). Driver «ferdig
  // bygget»-tilbudet under «Forespør om bygging».
  const area = (c.bredde * c.dybde) / 10000
  const sizeN = Math.min(1, Math.max(0, (area - 0.6) / (16 - 0.6)))
  let arbeidstimer = 16 + sizeN * 48 // 16 t (liten) → 64 t (stor grunnflate)
  if (c.taktype === 'valmtak') arbeidstimer += 12
  else if (c.taktype === 'saltak') arbeidstimer += 6
  arbeidstimer += (doorCount(c) - 1) * 4 // ekstra dørrom + skillevegg
  arbeidstimer = Math.round(Math.min(80, Math.max(16, arbeidstimer)))

  return { linjer, estimatKr, sammendrag, arealM2: (c.bredde * c.dybde) / 10000, maal: `${c.bredde} × ${c.dybde} × ${c.hoyde} cm`, arbeidstimer }
}

function kappliste(c: VedskjulConfig): KapplisteDel[] {
...
function tegning2D(c: VedskjulConfig): Tegning2D {
  const B = c.bredde
  const D = c.dybde
  const { nBjelke } = geo(c)
...
function buildMesh(c: VedskjulConfig, opts?: BuildOptions): THREE.Group {
  ...
  // ── STEG 2b: Front – bærende skillevegger + dører ──
  // Fronten deles i N like fag (ett per dør). Den doble toppdrageren (drager-
  // front) er dragerbjelken over åpningene; den bæres av hjørnestolpene og av
  // én gjennomgående BÆRINGSSTENDER ved hver skillevegg (SINTEF Byggforsk –
  // alle dører får opplegg i hver side, ingen fri drager over flere fag).
  const N = doorCount(c)
  const bayW = w / N
  const { leafW, karmDx } = doorLayout(bayW)
  const dInner = d - 2 * ST
  const frontZ = d / 2 - ST / 2 // frontveggens plan (senter i toppdrageren)
  const frontWallTop = deckTop + SW + postH // underkant toppdrager

  // Front bunnsvill (terskel) i full bredde.
  woodBox(w, SW, ST, 0, deckTop + SW / 2, frontZ, 'svill-bunn-f', 'Bunnsvill front (terskel)', STUD_PROFIL, 0.8)

  // Skillevegger mellom dørrommene + bæringsstender i front under toppdrageren.
  const buildDivider = (xk: number, di: number) => {
    woodBox(ST, SW, dInner, xk, deckTop + SW / 2, 0, `skille-${di}-svill`, 'Skillevegg (svill)', STUD_PROFIL, 0.82)
    woodBox(ST, SW, dInner, xk, frontWallTop + SW / 2, 0, `skille-${di}-topp`, 'Skillevegg (toppsvill)', STUD_PROFIL, 0.82)
    const nz = Math.max(2, antallCC(dInner))
    const zAt = (i: number) => -dInner / 2 + (i * dInner) / (nz - 1)
    for (let i = 0; i < nz; i++) {
      woodBox(ST, postH, SW, xk, postCY, zAt(i), `skille-${di}-stud-${i}`, 'Skillevegg (stender)')
    }
    // Losholt (kryss) mellom skilleveggstenderne, c/c 600.
    for (let r = 1; r <= nLosholt; r++) {
      const y = deckTop + SW + r * CC
      for (let s = 0; s < nz - 1; s++) {
        const z0 = zAt(s) + SW / 2
        const z1 = zAt(s + 1) - SW / 2
        if (z1 - z0 < 0.02) continue
        woodBox(ST, SW, z1 - z0, xk, y, (z0 + z1) / 2, `skille-${di}-los-${r}-${s}`, 'Losholt (kryss, skillevegg)', STUD_PROFIL, 0.84)
      }
    }
    // Gjennomgående bæringsstender i frontplanet – bærer toppdrageren.
    woodBox(SW, postH, ST, xk, postCY, frontZ, `skille-${di}-baere`, 'Bæringsstender (front)', STUD_PROFIL, 0.8)
  }
  for (let k = 1; k < N; k++) buildDivider(-w / 2 + k * bayW, k)

  // Dør: ramme (2 stolper + 3 tverrtre) + skråbånd, én per dørrom.
  const doorBot = deckTop + SW
  const doorH = postH
  const doorCY = doorBot + doorH / 2
  const doorZ = frontZ - ST / 2 + DOOR_T / 2 + 0.006 // like innenfor front (klaring)
  const buildDoor = (xC: number, di: number) => {
    const railW = Math.max(0.05, leafW - 2 * DOOR_FACE)
    // Dørkarm: bæringsstender i hver side av åpningen (bærer toppdrageren).
    woodBox(SW, postH, ST, xC - karmDx, postCY, frontZ, `dorkarm-${di}-v`, 'Dørkarm (bæringsstender)', STUD_PROFIL, 0.82)
    woodBox(SW, postH, ST, xC + karmDx, postCY, frontZ, `dorkarm-${di}-h`, 'Dørkarm (bæringsstender)', STUD_PROFIL, 0.82)
    // Stolper (venstre/høyre)
    woodBox(DOOR_FACE, doorH, DOOR_T, xC - leafW / 2 + DOOR_FACE / 2, doorCY, doorZ, `dor-${di}-sv`, 'Dør (ramme)', STUD_PROFIL, 0.9, 'dor')
    woodBox(DOOR_FACE, doorH, DOOR_T, xC + leafW / 2 - DOOR_FACE / 2, doorCY, doorZ, `dor-${di}-sh`, 'Dør (ramme)', STUD_PROFIL, 0.9, 'dor')
    // Tverrtre (topp / midt / bunn)
    woodBox(railW, DOOR_FACE, DOOR_T, xC, doorBot + DOOR_FACE / 2, doorZ, `dor-${di}-rb`, 'Dør (ramme)', STUD_PROFIL, 0.9, 'dor')
    woodBox(railW, DOOR_FACE, DOOR_T, xC, doorCY, doorZ, `dor-${di}-rm`, 'Dør (ramme)', STUD_PROFIL, 0.9, 'dor')
    woodBox(railW, DOOR_FACE, DOOR_T, xC, doorBot + doorH - DOOR_FACE / 2, doorZ, `dor-${di}-rt`, 'Dør (ramme)', STUD_PROFIL, 0.9, 'dor')
    // Skråbånd (Z-avstiving) mellom tverrtrærne
    const bw = railW
    const bh = doorH - 2 * DOOR_FACE
    if (bw > 0.05 && bh > 0.05) {
      const bl = Math.hypot(bw, bh)
      const bm = new THREE.Mesh(new THREE.BoxGeometry(bl, DOOR_FACE * 0.8, DOOR_T * 0.9), meshMat(`dor-${di}-brace`, 0.9))
      bm.position.set(xC, doorCY, doorZ - 0.002)
      bm.rotation.z = Math.atan2(bh, bw)
      bm.castShadow = true
      bm.receiveShadow = true
      bm.userData.part = 'dor'
      bm.userData.pid = `dor-${di}-brace`
      bm.userData.info = { navn: 'Dør (skråbånd)', profil: STUD_PROFIL, lengdeCm: Math.round(bl * 100) }
      group.add(bm)
    }
  }
  for (let j = 0; j < N; j++) buildDoor(-w / 2 + (j + 0.5) * bayW, j)

  // Front-felt ved siden av dørene: mellomstendere + losholt (kryss) som resten
  // av veggene (c/c ≤ 600). Feltene mellom dørkarm og hjørne/skillevegg.
  const buildFrontFraming = (xL: number, xR: number, si: number) => {
    const seg = xR - xL
    if (seg < 0.12) return
    const nb = Math.max(1, Math.ceil(seg / CC))
    const step = seg / nb
    for (let i = 1; i < nb; i++) {
      woodBox(SW, postH, ST, xL + i * step, postCY, frontZ, `fstud-${si}-${i}`, 'Stender (front)')
    }
    for (let r = 1; r <= nLosholt; r++) {
      const y = deckTop + SW + r * CC
      for (let s = 0; s < nb; s++) {
        if (step - SW < 0.02) continue
        woodBox(step - SW, SW, ST, xL + (s + 0.5) * step, y, frontZ, `flos-${si}-${r}-${s}`, 'Losholt (kryss, front)', STUD_PROFIL, 0.84)
      }
    }
  }
  frontSegments(w, N).forEach((sg, si) => buildFrontFraming(sg.xL, sg.xR, si))
...
export const vedskjul: ProductTemplate<VedskjulConfig> = {
  id: 'vedskjul',
  navn: 'Vedskjul',
  ikon: 'faFireBurner',
  beskrivelse: 'Tegn vedskjulet i 3D – med front-dører og bærende skillevegger, bygget opp steg for steg.',
  bilde: '/images/products/fire_wood_shed_1.webp',
  tilgjengelig: true,
  fraPris: 349,
  ...
  presets: [
    {
      ...
      config: { bredde: 360, dybde: 180, hoyde: 200, taktype: 'pulttak', takretning: 'bak', takvinkel: 15, takpapp: true, kledningsbord: 'lekt', dorer: '2', treslag: 'impregnert', farge: 'morkegra' },
    },
    {
      id: 'saltak-300',
      navn: 'Saltak 300 cm',
      beskrivelse: 'Klassisk, sortmalt smalkledning',
      config: { bredde: 300, dybde: 200, hoyde: 210, taktype: 'saltak', moneretning: 'dybde', takvinkel: 27, takpapp: true, kledningsbord: '19x98', dorer: '2', treslag: 'gran', farge: 'sort' },
    },
    {
      id: 'valmtak-420',
      navn: 'Valmtak 420 cm',
      beskrivelse: 'Eksklusiv lerk, bred kledning',
      config: { bredde: 420, dybde: 240, hoyde: 220, taktype: 'valmtak', takvinkel: 22.5, takpapp: true, kledningsbord: '19x148', dorer: '3', treslag: 'lerk', farge: 'ubehandlet' },
    },
  ],
  dimensjoner: [
    { key: 'bredde', label: 'Bredde', min: 120, max: 500, step: 5, axis: 'x' },
    { key: 'dybde', label: 'Dybde', min: 50, max: 400, step: 5, axis: 'z' },
    { key: 'hoyde', label: 'Vegghøyde', min: 150, max: 230, step: 5, axis: 'y' },
